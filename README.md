...
- LINEAE-2XL ONNX (1.5 GB) + TensorRT BF16 + RTX3070 - After fine-tuning with data from 200,000 composite screws
  ...
  - It is expected behavior that screws are not detected, as straight lines shorter than 5 pixels are excluded from the learning process.
...
| Variant | No-distillation config | Epochs | Steps | Distillation config | Epochs | Steps |
| --- | --- | --: | --: | --- | --: | --: |
...
| 2XL | `configs/lineae/lineae_2xl.py` | 50 | 31,250 | `configs/lineae/distill/lineae_2xl.py` | 50 | 31,250 |
...
`tools/benchmark.py` first applies LINEA's fused deploy conversion, then records FLOPs/MACs, parameters, peak memory, raw samples, and batch-1 p50/p95 Torch latency. `tools/export_onnx.py` exports the same fused model, validates it with `onnx.checker`, and simplifies it with onnxsim by default (`--disable-onnxsim` is available for diagnosis); it does not start ONNX Runtime or perform numerical parity. Models below protobuf's 2 GiB limit remain one `.onnx` file. Larger models automatically use a colocated `<name>.onnx.data` weight file. The onnxsim pass skips shape inference to preserve the external-data graph; with `--disable-onnxsim`, the graph is left unchanged and external tensors are streamed into the same single-file layout without shape inference. After either path succeeds, per-tensor external files created by the initial PyTorch export, including `Constant` attribute tensors, are removed automatically. By default, the exporter retains every query defined by the selected variant's `num_queries`. Supplying `--num-select K` (or its `--topk K` alias) explicitly filters the outputs to any validated `1 <= K <= num_queries` value and embeds K into the fixed ONNX output shapes. A normal export writes only these ONNX artifacts; `--report PATH` opt-in generates the hash-bound JSON report required by the qualification workflow. `tools/benchmark_tensorrt.py` separately builds with TF32 disabled, measures the engine, and gates its actual FP16 outputs against ONNX Runtime using the query/endpoint-order-invariant comparison. `tools/evaluate_onnx.py` separately enforces full-dataset deployment sAP5/10/15 parity against the hash-matched PyTorch evaluation before TensorRT benchmarking; its CUDA ORT mode disables TF32 and CPU execution-provider fallback. For a custom export top-k, create the PyTorch report with the same `tools/evaluate_checkpoint.py --num-select K`; `tools/evaluate_onnx.py` reads K from an explicitly requested export report. `tools/analyze_pareto.py` identifies non-dominated variants, and `tools/generate_model_card.py` turns archived reports into model cards. It requires a hash-matched Pareto report and refuses to label a dominated model as a qualified candidate.

When `--num-select` is omitted or equals the selected config's `num_queries`, the exporter passes all detector queries through in their original order and omits the output-selection `TopK` and `Gather` nodes. Internal proposal-selection `TopK` nodes remain part of the detector architecture. An optional export report records this distinction as `output_selection=all_queries_passthrough` instead of `class0_topk`.

### Interpreting 3XL checkpoint-to-ONNX numerical differences

The 3XL checkpoint and its ONNX export are not expected to be elementwise identical query by query, and a strict `allclose` failure alone does not mean that the external-data model is corrupt or that export reduced detector accuracy. The 3XL decoder is unusually sensitive to ordinary float32 changes because it combines 1,100 ranked proposals, rank-specific learned query embeddings, global self-attention, line `GridSample`, and ten iterative reference-point updates. The exporter also calls `model.deploy()`, which preserves checkpoint keys and equations but replaces the sequence-first inference implementation with the batch-first packed-attention path. PyTorch, ONNX Runtime, execution-provider, and runtime-optimizer kernels may consequently use different valid reduction and fusion orders.

The current 640x640/1,100-query investigation excluded weight and serialization failures: the checkpoint strict-loaded with no missing or unexpected keys, sampled matching ONNX initializers were bit-exact, `onnx.checker` accepted the graph and its consolidated `.onnx.data`, and backbone/encoder boundary tensors remained within normal float32 tolerance. The first important discontinuity was the decoder's internal proposal `TopK`. Across three deterministic inputs, PyTorch and ONNX selected the same 1,100-proposal set, while only 4--8 ranked positions differed. One exchanged pair had exactly equal float32 scores and another differed by only `6.68e-6`. ONNX `TopK` uses the lower source index to break an exact tie, whereas PyTorch does not guarantee stable indices for tied elements. Because each proposal rank is paired with a different learned query embedding, an adjacent exchange is not merely an output permutation: self-attention spreads it across queries and the recurrent decoder amplifies it. Forcing both runtimes to use the same proposal order reduced the mean line-coordinate difference by approximately 92--97%.

Line attention is an amplifier rather than evidence of a broken ONNX `GridSample`. With identical value and grid inputs, the measured PyTorch/ONNX Runtime `GridSample` mean difference was `1.56e-8`; the larger downstream difference came from tiny sampling-coordinate changes being interpolated on the feature maps and fed into later decoder layers. Likewise, onnxsim and onnxslim versions of the same 3XL graph produced bit-exact final outputs for seeds 0/1/2 with ONNX Runtime 1.26 CPU `ORT_ENABLE_ALL`, despite having different stored node counts. These observations explain why raw tensor deltas can look large while the model files and runtime execution remain valid.

LINEAE predictions are evaluated as scored, undirected line sets rather than as fixed query identities. Deployment correctness must therefore be decided after identical class-0 top-`num_select` postprocessing with full-dataset sAP5/10/15, using `tools/evaluate_onnx.py` against the hash-matched `tools/evaluate_checkpoint.py` report. A passing parity report establishes that the numerical differences are operationally harmless; elementwise equality is intentionally not the qualification criterion. Conversely, checker success or the diagnostic observations above must not be used to claim accuracy parity for an otherwise unevaluated ONNX artifact.

Deployment uses a batch-first decoder path without changing the training path or checkpoint keys. Packed self-attention reuses the existing `nn.MultiheadAttention` parameters, coordinate heads avoid scalar gather/stack chains, and line attention keeps `[batch, heads, channels, positions]` explicit except at the required `GridSample` boundary. The exporter prints the simplified graph's total, `Transpose`, `Gather`, and `Unsqueeze` node counts and rejects non-standard operator domains; exported graphs remain portable opset-17 ONNX.
