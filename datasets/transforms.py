...
from torchvision.transforms.v2 import RandomPhotometricDistort as TVRandomPhotometricDistort
import numbers
...
def crop(image, target, region):
    ...
    if "lines" in target:
        ...
        for index, line in enumerate(cropped_lines):
            clipped = _clip_segment_to_rectangle(line, w, h)
...
class RandomPhotometricDistort(object):
    """Gazelle-derived image-only distortion that leaves line geometry intact."""

    def __init__(self, p=0.5):
        if not 0.0 <= p <= 1.0:
            raise ValueError("photometric distortion probability must be in [0, 1]")
        self.transform = TVRandomPhotometricDistort(p=p)
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "lines" in target:
            lines = target["lines"]
            lines = lines / torch.tensor(
                [w, h, w, h],
                dtype=torch.float32,
                device=lines.device,
            )
            # Segment clipping can leave machine-epsilon excursions such as
            # -5e-17 at an image boundary. The detector contract is normalized
            # [0, 1], so remove only that numerical residue here.
