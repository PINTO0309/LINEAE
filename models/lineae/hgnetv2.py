...
class HGNetv2(nn.Module):
    ...
    def __init__(self,
        ...
        if pretrained:
            model_path = weights_path or os.path.join(
                local_model_dir, f'PPHGNetV2_{name}_stage1.pth'
            )
            if not os.path.isfile(model_path):
                raise FileNotFoundError(
                    f'HGNetV2 checkpoint not found: {model_path}. '
                    f'Download it explicitly from {download_url}; LINEAE never auto-downloads weights.'
                )
...
def build_hgnetv2(args):
    ...
    return HGNetv2(
        ...
        local_model_dir=getattr(args, 'checkpoint_dir', 'ckpts'),
        )
