...
class HybridEncoderAsymConv(nn.Module):
    def __init__(
        ...
        self._position_embedding_cache = {}
        self._position_embedding_cache_limit = 16
        self._fixed_position_shapes = {}
    ...
    def _reset_parameters(self):
        ...
        if self.eval_spatial_size is not None:
            for idx in self.use_encoder_idx:
                ...
                self._fixed_position_shapes[idx] = (
                    self.eval_spatial_size[0] // stride,
                    self.eval_spatial_size[1] // stride,
                )
    ...
    def _position_embedding_for(self, enc_idx, height, width, device):
        device = torch.device(device)
        fixed_shape = self._fixed_position_shapes.get(enc_idx)
        if not self.training and fixed_shape == (height, width):
            return getattr(self, f'pos_embed{enc_idx}')
        ...
        cached = self._position_embedding_cache.pop(cache_key, None)
        if cached is None:
            # Keep the legacy argument order so the first value is numerically
            # identical on its native device; LINEAE recipes use square inputs.
            cached = self.create_sinehw_position_embedding(
                height,
                width,
                self.hidden_dim // 2,
                device=device,
            )
            if len(self._position_embedding_cache) >= self._position_embedding_cache_limit:
                oldest_key = next(iter(self._position_embedding_cache))
                self._position_embedding_cache.pop(oldest_key)
        self._position_embedding_cache[cache_key] = cached
        return cached
    ...
    def forward(self, feats):
        ...
        for i, enc_idx in enumerate(self.use_encoder_idx):
            ...
            pos_embed = self._position_embedding_for(
                enc_idx,
                H_,
                W_,
                src_flatten.device,
