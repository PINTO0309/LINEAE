...
def ms_deform_attn_core_pytorchv2(value, value_spatial_shapes, sampling_locations, attention_weights, num_points_list):
    ...
    if torch.is_grad_enabled():
        ...
        # The concatenation is a fresh tensor with no other consumer. During
        # inference and frozen-teacher forwards, reuse its storage instead of
        # allocating an equally large product tensor. Point and reduction order
        # remain identical to the training/autograd path.
        sampled_values.mul_(attention_weights)
