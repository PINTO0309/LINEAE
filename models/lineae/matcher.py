...
from .linea_utils import pairwise_endpoint_l1
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        cost_bbox: float = 1,
        focal_alpha=0.25,
        endpoint_invariant_lines: bool = False,
    ):
        """Creates the matcher
        Params:
    ...
    def forward(self, outputs, targets):
        ...
        cost_line = (
            pairwise_endpoint_l1(out_line, tgt_line)
            if self.endpoint_invariant_lines
            else torch.cdist(out_line, tgt_line, p=1)
        )
