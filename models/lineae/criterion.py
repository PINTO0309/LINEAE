...
class LINEACriterion(nn.Module):
    ...
    def __init__(
        ...
        self.endpoint_invariant_lines = bool(endpoint_invariant_lines)
    ...
    def loss_lines(self, outputs, targets, indices, num_boxes):
        ...
        direct = F.l1_loss(src_lines, target_lines, reduction='none').sum(dim=-1)
        if self.endpoint_invariant_lines:
            swapped = F.l1_loss(
                src_lines,
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if self.training and dn_meta and dn_meta.get('pad_size', 0) > 0 and 'aux_denoise' in outputs:
            single_pad, scalar = self.prep_for_dn(dn_meta)
            dn_pos_idx = []
            dn_neg_idx = []
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    target_device = targets[i]['labels'].device
                    t = torch.arange(len(targets[i]['labels']), device=target_device).long()
                    t = t.unsqueeze(0).repeat(scalar, 1)
                    tgt_idx = t.flatten()
                    output_idx = (
                        torch.arange(scalar, device=target_device) * single_pad
                    ).long().unsqueeze(1) + t
                    output_idx = output_idx.flatten()
                else:
                    output_idx = tgt_idx = torch.empty(
                        0, dtype=torch.long, device=targets[i]['labels'].device
                    )
            ...
            if 'aux_pre_outputs' in dn_outputs:
                ...
                l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}  
                l_dict = {k + '_pre_dn': v for k, v in l_dict.items()}
                losses.update(l_dict)
...
class DFINESetCriterion(LINEACriterion):
    def __init__(
        ...
        super().__init__(
            ...
            focal_alpha,
            losses,
            endpoint_invariant_lines=endpoint_invariant_lines,
        )
    ...
    def forward(self, outputs, targets):
        ...
        if 'aux_lmap' in outputs:
            l_dict = self.get_loss('lmap', outputs, targets, indices, num_boxes)
...
def build_criterion(args):
    ...
    if args.criterion_type == 'default':
        criterion = LINEACriterion(
            ...
            losses=args.losses,
            endpoint_invariant_lines=getattr(args, 'endpoint_invariant_lines', False),
        )
