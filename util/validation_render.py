...
def validate_validation_render_options(args) -> None:
    ...
    max_predictions = int(
        getattr(args, "validation_render_max_predictions", 100)
    )
    if count < 0:
    ...
    if not 0.0 <= threshold <= 1.0:
        raise ValueError(
            "validation_render_score_threshold must be in [0, 1]"
        )
    ...
    if max_predictions > num_select:
        raise ValueError(
            "validation_render_max_predictions cannot exceed num_select"
        )
...
def _tensor_to_image(
    ...
    pixels = (
        pixels.permute(1, 2, 0).mul(255).round().to(torch.uint8).numpy()
    )
...
def save_best_validation_renders(
    *,
    ...
    temporary_dir = render_root / f".best_epoch_{int(epoch):04d}.tmp"
    ...
    try:
        ...
        for start in range(0, count, batch_size):
            ...
            target_sizes = torch.stack(
                [target["size"] for target in device_targets]
            )
            ...
            for image, target, prediction in zip(
                ...
                if not cv2.imwrite(str(destination), rendered):
                    raise RuntimeError(f"OpenCV could not write validation render: {destination}")
                saved_count += 1
